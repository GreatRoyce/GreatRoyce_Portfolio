...
function Projects() {
      ...
      try {
        ...
        const normalized = data.map((p) => ({
          ...
          images: p.images?.map((img) =>
            img.startsWith("http")
              ? img
              : `${API_BASE.replace("/api/v1", "")}${img}`,
          ),
          video: p.video
            ? `${API_BASE.replace("/api/v1", "")}${p.video}`
            : null,
  ...
  if (loading)
    return (
      <section
        className={`${isDarkMode ? "bg-gray-900" : "bg-gray-50"} min-h-screen flex items-center justify-center`}
      >
        <div className="text-center">
          ...
          <p
            className={`${isDarkMode ? "text-gray-300" : "text-gray-600"} mt-6`}
          >
            Loading projects...
          </p>
        </div>
